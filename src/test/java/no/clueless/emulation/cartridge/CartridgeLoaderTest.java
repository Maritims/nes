...
class CartridgeLoaderTest {
    ...
    public static Stream<Arguments> isValidHeaderSignature_should_return_false_for_invalid_header_signature() {
        return Stream.of(
                Arguments.of((Object) UnsignedByte.arrayOf(new byte[] { 'N', 'E', 'S', 0x1B, 0x1A })),
                Arguments.of((Object) UnsignedByte.arrayOf(new byte[] { 'N', 'E', 'S', 0x1B }))
        );
    }

    @Test
    void isValidHeaderSignature_should_return_true_for_valid_header_signature() {
        var bytes = UnsignedByte.arrayOf(new byte[] { 'N', 'E', 'S', 0x1A });
        assertTrue(CartridgeLoader.isValidHeaderSignature(bytes));
    }

    @ParameterizedTest
    @MethodSource
    void isValidHeaderSignature_should_return_false_for_invalid_header_signature(UnsignedByte[] bytes) {
