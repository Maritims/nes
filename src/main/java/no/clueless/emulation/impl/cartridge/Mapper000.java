...
public class Mapper000 implements Mapper {
    ...
    public OptionalInt mapCpuAddress(int address) {
        if (address >= CPU_ADDRESS_SPACE_START && address <= CPU_ADDRESS_SPACE_END) {
            ...
            return OptionalInt.of(mappedAddress);
        }

        return OptionalInt.empty(); // Address does not point to cartridge PRG memory
    }

    @Override
    public OptionalInt mapPpuAddress(int address) {
        // PPU reads pattern tables directly from $0000 to $1FFF (8KB)
        return address >= PPU_ADDRESS_SPACE_START && address <= PPU_ADDRESS_SPACE_END ? OptionalInt.of(address) : OptionalInt.empty();
    }
}