...
public class Tile {
    ...
    private final        int[][] pixelIndices = new int[HEIGHT][WIDTH];
     ...
     */
    public Tile(UnsignedByte[] bytes) {
        if (bytes.length != 16) {
        ...
        for (var row = 0; row < WIDTH; row++) {
            ...
            var plane0 = bytes[row].intValue() & 0xFF;

            // Row X of sheet 1 (high bits).
            var plane1 = bytes[row + 8].intValue() & 0xFF;

            for (var col = 0; col < HEIGHT; col++) {
                var lsb = getBitAtColumn(plane0, col);
                var msb = getBitAtColumn(plane1, col);
                pixelIndices[row][col] = combineBitsToColorIndex(msb, lsb);
            }
        }
    }
